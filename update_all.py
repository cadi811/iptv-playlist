...
COUNTRIES = {
    "ALXK": {
        ...
        "sources": [
            ...
            ("iptv-org Entertainment ALXK", "https://iptv-org.github.io/iptv/categories/entertainment.m3u"),
...
MANUAL_CANDIDATES = {
    "ALXK": [
        ...
        {
            "name": "RTK 1 Satelit",
            "url": "https://gjirafa-video-live.gjirafa.net/gjvideo-livestream/rtk-sat/index.m3u8",
            "meta": '#EXTINF:-1 tvg-id="RTK1.xk" group-title="Albanische Sender",RTK 1 Satelit',
            "source": "Öffentlicher Gjirafa-Fallback 2026",
        },
        {
            "name": "RTK 1",
            "url": "https://gjirafa-video-live.gjirafa.net/gjvideo-livestream/98r-d35-487-v6m/tracks-v4a1/mono.m3u8",
            "meta": '#EXTINF:-1 tvg-id="RTK1.xk" group-title="Albanische Sender",RTK 1',
            "source": "Früherer öffentlicher IPTV-Org-Fallback",
        },
        {
            "name": "Syri Vision HD",
            "url": "https://gjirafa-video-live.gjirafa.net/gjvideo-live/xej-xnb-ba0-kup/index.m3u8",
            "meta": '#EXTINF:-1 group-title="Albanische Sender",Syri Vision HD',
            "source": "Öffentlicher Gjirafa-Fallback 2026",
        },
        {
            "name": "TVA News",
            "url": "https://live.tvanews.com/live/tvanews/chunklist_w869460556.m3u8",
            "meta": '#EXTINF:-1 group-title="Albanische Sender",TVA News',
            "source": "Öffentlicher Sender-Fallback",
        },
        {
            "name": "RTV Ahireti",
            "url": "https://tv.rtvahireti.com/Ahierti/live/playlist.m3u8",
            "meta": '#EXTINF:-1 group-title="Albanische Sender",RTV Ahireti',
            "source": "Öffentlicher Sender-Fallback",
        },
        {
            "name": "RTV 1",
            "url": "http://video.radio1.al/hls/rtvnje/index.m3u8",
            "meta": '#EXTINF:-1 group-title="Albanische Sender",RTV 1',
            "source": "Öffentlicher Sender-Fallback",
        },
        {
            "name": "TV Prizreni",
...
def is_excluded(code, name):
    n = norm(name)
    if code == "ALXK":
        return n in EXCLUDE_EXACT_ALXK or any(k in n for k in EXCLUDE_KEYWORDS_ALXK)
    if code in {"DE", "AT"}:
...
def is_alxk_category_entry(item):
    """Für globale iptv-org Kategorien nur klar AL/XK/SQI zuordenbare Einträge zulassen."""
    meta = item.get("meta", "")
    low = meta.casefold()
    tvg_id_match = re.search(r'tvg-id="([^"]*)"', meta, flags=re.I)
    tvg_id = tvg_id_match.group(1).casefold() if tvg_id_match else ""
    return (
        'tvg-country="al"' in low
        or 'tvg-country="xk"' in low
        or 'tvg-language="sqi"' in low
        or tvg_id.endswith(".al")
        or tvg_id.endswith(".xk")
    )
...
def collect_country(code, cfg):
    ...
    for label, url in cfg["sources"]:
        try:
            fetched = parse(fetch(url), label)
            if code == "ALXK" and label.startswith("iptv-org ") and label.endswith(" ALXK"):
                fetched = [c for c in fetched if is_alxk_category_entry(c)]
            candidates += fetched
        except Exception as e:
